...
def get_system_identity(snapshot):
    info = {}
    try:
        # Read the OS product name from registry; this identifies the Windows version (e.g., "Windows 11 Pro")
        info["os_name"] = get_registry_value(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Windows NT\CurrentVersion", "ProductName")
        
        # Read the build number; this is a string like "22631" for precise version tracking
        info["os_build"] = get_registry_value(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Windows NT\CurrentVersion", "CurrentBuild")
        
        # Read the edition ID; this shows the license type (e.g., "Enterprise") for feature/capability assessment
        info["os_edition"] = get_registry_value(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Windows NT\CurrentVersion", "EditionID")
        
        # Read the registered owner; this is often the installer username, useful for ownership verification
        ...
        # Read InstallDate as a Unix timestamp integer; convert to ISO UTC string for readability (choice: ISO format with Z suffix for UTC clarity, could use local time but UTC is standard for logs)
        ...
        if install_epoch is not None:
            ...
            info["install_date_utc"] = None  # Leave as None if missing, as some systems don't store it
    except Exception as e:
...
def add_running_processes(snapshot):
    try:
        # Run tasklist in CSV mode and parse the output instead of using subprocess.run directly.
        output = run_command(["tasklist", "/fo", "csv"])

        # Create a CSV reader for the output lines so quoted values are parsed correctly.
        reader = csv.reader(output.splitlines())

        # Skip the header row, because it contains column names instead of process data.
        header = next(reader, None)
        
        # Walk every remaining CSV row and append a dict for each process.
        for row in reader:
            if len(row) < 2:
                continue  # Skip malformed rows that do not contain at least name and PID.

            name = row[0].strip()
            pid_text = row[1].strip()

            # Convert the PID from string to int. If conversion fails, store None.
            try:
                pid = int(pid_text)
            except ValueError:
                pid = None
            ...
            snapshot["running_processes"].append({
                "pid": pid,
                "name": name,
                "parent_pid": None,      # tasklist does not include parent PID in this output.
                "executable_path": None, # tasklist does not provide the executable path here.
                "command_line": None,    # tasklist does not provide command-line arguments here.
            })
    except Exception as e:
